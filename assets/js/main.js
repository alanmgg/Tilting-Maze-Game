...
function resetGame() {
  ...
  if (hardMode) {
    noteElement.innerHTML = `Haga clic en el joystick para comenzar!
          <p>Modo difícil, evita agujeros negros. ¿Volver al modo fácil? Presione E</p>`;
  } else {
    noteElement.innerHTML = `Haga clic en el joystick para comenzar!
...
function main(timestamp) {
  ...
  try {
    ...
    if (
      ...
      noteElement.innerHTML = `Felicidades, lo hiciste!
          ${!hardMode ? "<p>Presiona H para modo difícil</p>" : ""}
          <p>
            Sígueme
    ...
    if (error.message == "The ball fell into a hole") {
      noteElement.innerHTML = `¡Una pelota cayó en un agujero negro! Presiona espacio para reiniciar el juego.
          <p>
          ¿Volver a lo fácil? Presione E
          </p>`;
      noteElement.style.opacity = 1;
      gameInProgress = false;
    } else throw error;
